...
| 可信知识库 | TXT / Markdown / CSV / PDF / PPTX 导入预览、来源指纹、修订链、人工确认与审核 |
| AI 内容创作 | 短视频、手机拍摄清单、直播、评论回复、社交文案、标题与封面文案；已提供零成本生成器与 OpenAI-compatible 真实模型适配器 |
| 营销材料导出 | 将活动、品牌、产品、卖点、内容矩阵与审核状态生成可继续编辑的 16:9 PPTX |
...
docs/
  ...
  open-source-adapters.md      第三方开源能力的许可证审查与平台适配原则
...
| 建设方向 | 已有基础 | 下一步实施方式 |
| --- | --- | --- |
| PDF / PPTX 资料导入深化 | 已实现 15 MB 受限上传、PDF / PPTX 本地文字抽取、页或幻灯片分段、解析提示、编辑预览和确认后入库 | 下一步为扫描 PDF 增加可选 OCR、表格与图片说明抽取、页级引用定位和批量导入；默认仍不把原文件上传到外部服务 |
| 营销方案 PPTX 生成深化 | 已实现稳定数据结构、“日光极光”母版、原生可编辑 16:9 PPTX、封面、摘要、卖点、内容矩阵、来源审核页、长文本保护和下载接口 | 下一步增加多套品牌模板、内容排期页、逐条脚本详情页、用户上传母版映射、导出前预览和 PDF 导出；复杂模板适配继续采用许可证兼容的开源库 |
| 文档与导出审计 | 已计算导入文件 SHA-256，并在 PPTX 中展示来源与聚合审核状态；未完整审核的材料会明确标记为草稿 | 建立不可变的文档导入与导出记录，保存原文件指纹、页级来源、解析器版本、确认人、确认时间、内容与知识版本、模板版本、导出人、导出时间及导出文件 SHA-256；正式审核版与内部草案使用不同导出门禁 |
...
| 供应链与许可证治理 | 已记录当前文档能力的上游项目、用途和许可证，并通过仓库审计阻止密钥、数据库及私密资料进入版本库 | 锁定生产依赖，自动生成 SBOM 与 `THIRD_PARTY_NOTICES`，持续检查许可证兼容性、已知漏洞、维护状态和依赖体积 |
