...
> 不参与开发也可以本地体验。无需 Docker、Ollama、Node.js、域名或付费模型 API。

1. 接受 GitHub 私有仓库邀请；
2. 在仓库页面选择 **Code → Download ZIP**，解压到剩余空间充足的目录（建议 D 盘）；
3. 确认电脑已经安装 **Python 3.12**；
4. 第一次双击 `安装禾语AI.bat`，等待依赖安装完成；
5. 以后双击 `启动禾语AI.bat`，浏览器会打开平台。
