...
> 需要马上向负责人或视频同学展示时，请直接按照 [下午演示指引](docs/demo-showcase.md) 操作。
...
5. 每条视频的开头、正文、封面文案、背景乐方向、分镜、手机拍摄提示、规则质量分和可执行改进建议；
6. 一套直播讲解结构；
7. 七天发布与运营计划；
...
热点辅助当前用于**选题规划与适配判断**，会记录信号类型并明确提示它不是实时热度证明。平台不会虚构热度数值，也不会承诺爆款或销量提升。
