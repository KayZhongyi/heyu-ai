...
async function renderReviews(selectedId){
  ...
  $("#review-list").innerHTML=state.versions.map(v=>{
    const readable=HeyuContent.renderContent(v.content);
    const actions=v.status==="draft"&&canSubmit
      ? `<div class="row-actions"><button class="approve" data-submit-version="${v.id}" data-project="${v.project_id}">提交审核</button></div>`
      : v.status==="pending_review"&&canReview
        ? `<div class="content-review-form"><label for="review-note-${v.id}">审核意见 <span>通过时选填，驳回时必填</span></label><textarea id="review-note-${v.id}" data-review-note="${v.id}" maxlength="2000" rows="3" placeholder="记录事实核对结果、修改建议或驳回原因"></textarea><small><span data-review-count="${v.id}">0</span> / 2000</small><div class="row-actions"><button class="approve" data-review-version="${v.id}" data-project="${v.project_id}" data-status="approved">审核通过</button><button class="reject" data-review-version="${v.id}" data-project="${v.project_id}" data-status="rejected">驳回修改</button></div></div>`
        : "";
    return `<article class="content-review-card"><div class="panel-heading"><div><p class="eyebrow">VERSION ${v.version_number}</p><h3>${escapeHtml(v.change_summary||"AI 初稿")}</h3></div><span class="badge ${v.status}">${escapeHtml(statusLabels[v.status]||v.status)}</span></div><div class="review-reading-copy">${escapeHtml(readable)}</div><details class="review-structured"><summary>查看结构化 JSON</summary><pre>${escapeHtml(JSON.stringify(v.content,null,2))}</pre></details>${v.review_note?`<p class="review-note"><strong>审核意见</strong>${escapeHtml(v.review_note)}</p>`:""}${actions}</article>`;
  }).join("")||"暂无内容版本";
}
