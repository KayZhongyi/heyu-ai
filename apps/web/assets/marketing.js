(() => {
    ...
    tea: {
      ...
      content: {
        "zh-CN": {
          productName: "高山云雾茶",
          origin: "贵州黔南",
          description: "茶园位于高山云雾带，春季分批采摘嫩叶，由合作社统一摊晾、制作和分级。茶汤清爽，带自然花香，适合日常清饮。",
          sellingPoints: "高山茶园，春季嫩叶，合作社分级，清爽花香",
          audience: "喜欢日常喝茶、关注产地与制作过程的城市消费者",
          trend: "办公室轻养生与春日饮茶"
        },
        "zh-HK": {
          productName: "高山雲霧茶",
          origin: "貴州黔南",
          description: "茶園位於高山雲霧帶，春季分批採摘嫩葉，由合作社統一攤晾、製作及分級。茶湯清爽，帶天然花香，適合日常清飲。",
          sellingPoints: "高山茶園，春季嫩葉，合作社分級，清爽花香",
          audience: "喜歡日常品茶、重視產地與製作過程的都市消費者",
          trend: "辦公室輕養生與春日品茶"
        },
        en: {
          productName: "High-mountain mist tea",
          origin: "Qiannan, Guizhou",
          description: "Tender spring leaves are picked in batches from misty high-altitude gardens, then rested, processed and graded by the cooperative. The tea is clean and refreshing with a natural floral note.",
          sellingPoints: "high-altitude gardens, spring leaves, cooperative grading, floral finish",
          audience: "urban tea drinkers who care about origin and craft",
          trend: "mindful office breaks and spring tea rituals"
        }
      }
    },
    fruit: {
      ...
      content: {
        "zh-CN": {
          productName: "岭南当季水果礼盒",
          origin: "广东茂名",
          description: "根据当周成熟情况组合本地当季水果，由乡村运营团队与果园确认采摘批次、分选标准和发货安排。适合家庭分享与节日送礼。",
          sellingPoints: "当季组合，果园直采，按批次分选，适合分享",
          audience: "关注应季风味、希望了解果园故事的家庭和送礼人群",
          trend: "周末果园与应季伴手礼"
        },
        "zh-HK": {
          productName: "嶺南時令水果禮盒",
          origin: "廣東茂名",
          description: "按每週成熟情況組合本地時令水果，由鄉村營運團隊與果園確認採摘批次、分選標準及送貨安排。適合家庭分享及節日送禮。",
          sellingPoints: "時令組合，果園直採，按批次分選，適合分享",
          audience: "重視時令風味、希望了解果園故事的家庭及送禮人士",
          trend: "週末果園與時令伴手禮"
        },
        en: {
          productName: "Lingnan seasonal fruit box",
          origin: "Maoming, Guangdong",
          description: "A changing selection of local fruit based on what is ripe each week. The rural operations team confirms harvest batches, grading standards and fulfilment with partner orchards.",
          sellingPoints: "seasonal selection, orchard sourced, batch graded, made for sharing",
          audience: "families and gift buyers who value seasonal flavour and orchard stories",
          trend: "weekend orchard stories and seasonal gifting"
        }
      }
    }
  };
  ...
  function applyLocale(next) {
    ...
    localStorage.setItem("heyu-locale", locale);
  ...
  function setDemoStatus(state, caseName = "") {
    const messages = {
      "zh-CN": {
        loading: `${caseName}案例已填充，正在生成预览…`,
        ready: `${caseName}案例预览已就绪，可继续修改表单。`,
        failed: `${caseName}案例已填充，预览生成失败，请重试。`
      },
      "zh-HK": {
        loading: `${caseName}案例已填妥，正在生成預覽…`,
        ready: `${caseName}案例預覽已準備好，可繼續修改表單。`,
        failed: `${caseName}案例已填妥，預覽生成失敗，請重試。`
      },
      en: {
        loading: `${caseName} is filled in. Generating the preview…`,
        ready: `${caseName} preview is ready. You can still edit the brief.`,
        failed: `${caseName} is filled in, but its preview failed. Please try again.`
      }
    };
    demoStatus.textContent = messages[locale][state];
  }

  function fillDemoCase(caseId) {
    const demo = demoCases[caseId];
    const values = demo?.content[locale];
    ...
    document.querySelectorAll("[name=platform]").forEach((input) => {
      input.checked = input.value === demo.platform;
    });
    form.elements.tone.value = demo.tone;
    form.elements.product_name.value = values.productName;
    form.elements.origin.value = values.origin;
    form.elements.product_description.value = values.description;
    form.elements.selling_points.value = values.sellingPoints;
    form.elements.audience.value = values.audience;
    form.elements.trend.value = values.trend;
    document.querySelector("#description-count").textContent = values.description.length;

    demoButtons.forEach((button) => {
      button.classList.toggle("active", button.dataset.demoCase === caseId);
    });
    const caseName = copy[locale][`demo${caseId[0].toUpperCase()}${caseId.slice(1)}`];
    setDemoStatus("loading", caseName);
    form.requestSubmit();
  }
  ...
  async function readApiError(response) {
    ...
    try {
      ...
      // The localized generic message below is safer than exposing a raw server response.
    }
  ...
  document.querySelector("[name=product_description]").addEventListener("input", (event) => document.querySelector("#description-count").textContent = event.target.value.length);
  document.querySelectorAll("[name=goals]").forEach((input) => input.addEventListener("change", () => {
    const checked = [...document.querySelectorAll("[name=goals]:checked")];
    if (checked.length > 3) input.checked = false;
  }));
  demoButtons.forEach((button) => button.addEventListener("click", () => fillDemoCase(button.dataset.demoCase)));
  form.addEventListener("input", () => {
    if (!selectedDemo) return;
    selectedDemo = null;
    demoButtons.forEach((button) => button.classList.remove("active"));
    demoStatus.innerHTML = copy[locale].demoCasesHint;
  });

  form.addEventListener("submit", async (event) => {
    event.preventDefault();
    const button = form.querySelector("[type=submit]");
    const submittedDemo = selectedDemo;
    const caseName = submittedDemo ? copy[locale][`demo${submittedDemo[0].toUpperCase()}${submittedDemo.slice(1)}`] : "";
    ...
    const payload = {
      ...
      product_name: values.get("product_name").trim(),
      origin: values.get("origin").trim(),
      product_description: values.get("product_description").trim(),
      selling_points: values.get("selling_points").split(/[，,]/).map((item) => item.trim()).filter(Boolean),
    ...
    try {
      ...
      if (submittedDemo) setDemoStatus("failed", caseName);
      alert(error instanceof Error ? error.message : copy[locale].genericError);
      console.error(error);
    } finally {
      button.disabled = false;
      demoButtons.forEach((demoButton) => demoButton.disabled = false);
